...
/*
  ...
  /*@Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Use MockApiService or real ApiService based on a flag
        boolean useMockApi = false; // Set this to false to use the real API
        ...
        loginButton.setOnClickListener(v -> {
            ...
            // Validate credentials with the server
            Call<UserBoundary> call = apiService.loginUser("mockSystemId", email);
            call.enqueue(new Callback<UserBoundary>() {
                @Override
                public void onResponse(Call<UserBoundary> call, Response<UserBoundary> response) {
                    if (response.isSuccessful() && response.body() != null) {
